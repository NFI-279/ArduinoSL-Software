...
const int pinSenzor = D1; // Declaram pin-ul D1 pentru DHT11
...
void loop() 
{
  //digitalWrite(pinBuzzer,HIGH); // Buzzer-ul este oprit
  ...
  if(Serial.available())  
  {
    ...
    if(readString.length() > 0)
    {
      ...
      switch(switchCase.c_str()[0])
      {
       ...
       // case 'M' : MaxWater = caseData.toDouble(); break;
       // case 'R' : WaterRefillLevel = caseData.toDouble(); break;
        //case 'P' : digitalWrite(pompa,LOW); pompaIsRunning = 1; break;
       // case 'O' : digitalWrite(pinBuzzer,LOW); Anulare() ;break;
...
// {
   ...
   if(sensorValue > 900 && WWorkMode == true)
    {
 ...
 // Trimiterea datelor catre SmartLauncher
  ...
  Serial.print((String) temperatura + "a" + umiditate + "b" + ddistanta + "c" + AAvailableWater + "d" + WaterRefill+ "e" + WaterDepth + "f" + pompaIsRunning + "g" + TankCapacity + "h" + WorkMode + "i" + "\n"); // Printam in Serial detaliile ce vor fi preluate de catre SmartLauncher pentru a fi afisate

  delay(500); // Asteptam 1 secunda pana la urmatorul ciclu
